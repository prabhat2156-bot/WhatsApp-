...
//   • 10 min inactivity → socket suspended (RAM freed), auth stays in MongoDB
...
const activeTasks  = new Set();   // userId → has long-running task running

function touch(userId) { lastActive.set(userId, Date.now()); }

// Mark user as "busy" — suspend will be skipped until task finishes
function markTaskStart(userId) { activeTasks.add(userId); touch(userId); }
function markTaskEnd(userId)   { activeTasks.delete(userId); touch(userId); }

// ─── Cleanup: every 30 seconds suspend inactive users ─────────────────────
// NOTE: We do NOT resetSession here — it would break mid-flow features.
// Telegram session is only reset when user explicitly goes to main menu.
setInterval(async () => {
  const now = Date.now();
  for (const [userId, t] of [...lastActive.entries()]) {
    if (now - t > SUSPEND_MS) {
      // Skip if user has an active long-running task (10-15 min operations)
      if (activeTasks.has(userId)) {
        lastActive.set(userId, now); // reset timer — task is still running
        continue;
      }
      const acctIdx = userAccountMap.get(userId);
      if (acctIdx !== undefined && getStatus(acctIdx) === "connected") {
        await suspendAccount(acctIdx).catch(() => {});
      }
      lastActive.delete(userId);
      // Do NOT call resetSession — preserve user's feature flow state
    }
  }
...
// ─── ensureConnected: use this in EVERY feature handler ─────────────────────
// Returns true if WhatsApp is connected (auto-resumes if suspended).
// Returns false and sends error message if not connected at all.
async function ensureConnected(uid, ctx) {
  const acctIdx = await uidxAsync(uid);   // always correct, never 0 fallback
  const status  = getStatus(acctIdx);
  if (status === "connected") return true;

  // If user had a saved phone (was suspended) → auto-resume with message
  if (indexPhoneMap.has(acctIdx)) {
    const phone = indexPhoneMap.get(acctIdx);

    // Show "Reconnecting..." message to user
    let reconnMsg = null;
    try {
      reconnMsg = await ctx.reply(
...
⏳ Please wait a moment...`,
    ...
    try {
      await resumeAccount(acctIdx, phone);
      // Wait up to 12s for connection (poll every 500ms)
      for (let i = 0; i < 24; i++) {
        await new Promise(r => setTimeout(r, 500));
        if (getStatus(acctIdx) === "connected") {
          // Show "Connected!" then delete both messages after 1.5s
          let doneMsg = null;
          try {
            doneMsg = await ctx.reply(
...
📱 +${getPhone(acctIdx)}`,
          ...
          // Delete reconnecting messages after short delay
          setTimeout(async () => {
            if (reconnMsg) {
              try { await bot.telegram.deleteMessage(ctx.chat.id, reconnMsg.message_id); } catch {}
            }
            if (doneMsg) {
              try { await bot.telegram.deleteMessage(ctx.chat.id, doneMsg.message_id); } catch {}
            }
          }, 1500);
          return true;
        }
    ...
    // Reconnect failed — update message to show error
    if (reconnMsg) {
      try {
        await bot.telegram.editMessageText(
          ctx.chat.id, reconnMsg.message_id, undefined,
          `❌ *Reconnect failed*

Please connect WhatsApp again.`,
          { parse_mode: "Markdown" }
        );
      } catch {}
    }
  ...
  // Not connected and no saved auth → show connect button
  if (ctx.callbackQuery) {
    await ctx.answerCbQuery("📱 WhatsApp not connected!", { show_alert: true }).catch(() => {});
  }
  await ctx.reply(
    `📱 *WhatsApp not connected*

Please connect your WhatsApp first.`,
    { parse_mode: "Markdown", ...Markup.inlineKeyboard([
      [Markup.button.callback("📱 Connect WhatsApp", "menu_account")],
...
bot.use(async (ctx, next) => {
  ...
  if (!uid) return next();
  touch(uid);
  // AWAIT to guarantee userAccountMap is populated before any handler runs
  // This is the critical fix — without await, uidx(uid) could return 0 (wrong)
  if (!userAccountMap.has(uid)) {
    await getOrCreateUserIdx(uid).catch(() => {});
  }
  return next();
});
...
async function startProgress(ctx, uid, text) {
  ...
  markTaskStart(uid);  // protect from auto-suspend during long operations
  return m;
}
...
bot.action("cancel_exec", async (ctx) => {
  ...
  markTaskEnd(ctx.from.id);  // task cancelled — allow suspend again
...
async function cancellableSleep(ms, uid) {
  ...
  while (elapsed < ms) {
    ...
    elapsed += step;
    // Keep user "alive" during long feature runs — activeTasks handles suspend
    // but touch ensures lastActive map doesn't get stale
    if (elapsed % 10000 < step) touch(uid);
  }
  return false;
}
...
async function sendSummary(ctx, opts) {
  ...
  if (uid) markTaskEnd(uid);  // task finished — allow suspend again
...
// ─── Main Menu ─────────────────────────────────────────────────────────────
async function buildMainMenu(uid) {
  const waIdx = await uidxAsync(uid);
  const c = getStatus(waIdx) === "connected", p = getPhone(waIdx);
...
async function sendMainMenu(ctx) {
  const uid = ctx.from?.id;
  touch(uid);
  const waIdx = uid ? await uidxAsync(uid) : 0;
  const c = getStatus(waIdx) === "connected", p = getPhone(waIdx);
  ...
  await ctx.reply(
    ...
    `╰───────────────────────╯\n\n` +
    `› Choose an option:`,
    { parse_mode: "Markdown", ...await buildMainMenu(uid) }
  );
}
...
bot.command("status", async (ctx) => {
  ...
  const waIdx = await uidxAsync(uid);
  const s = getStatus(waIdx), p = getPhone(waIdx);
  ...
  await ctx.reply(
    ...
    `⏱ Auto-suspend: *10 min inactivity*\n` +
    `━━━━━━━━━━━━━━━━━━━━`,
...
bot.action("menu_account", async (ctx) => {
  ...
  const uid = ctx.from.id; touch(uid);
  const waIdx = await uidxAsync(uid);
  let status = getStatus(waIdx), phone = getPhone(waIdx);
  // If disconnected but has saved auth (was suspended) → auto-resume silently
  if (status === "disconnected" && indexPhoneMap.has(waIdx)) {
    phone = indexPhoneMap.get(waIdx);
    await resumeAccount(waIdx, phone).catch(() => {});
    await sleep(2000);
    status = getStatus(waIdx);
  }
  if (status === "connected") {
    await editOrReply(ctx,
      `📱 *WhatsApp Account*\n━━━━━━━━━━━━━━━━━━━━\n✅ Connected\n📞 +${phone}\n━━━━━━━━━━━━━━━━━━━━\nLogout?`,
    ...
    updateSession(uid, { awaitingPhoneForIndex: waIdx });
    await editOrReply(ctx,
...
bot.action("logout_wa", async (ctx) => {
  ...
  await disconnectAccount(await uidxAsync(uid)); await sleep(800); await sendMainMenu(ctx);
});
bot.action("reset_wa", async (ctx) => {
  await ctx.answerCbQuery("Resetting...");
  const uid = ctx.from.id; touch(uid);
  const waIdx = await uidxAsync(uid);
  await disconnectAccount(waIdx);
  updateSession(uid, { awaitingPhoneForIndex: waIdx });
  await editOrReply(ctx,
...
Object.keys(FEAT_MAP).forEach((key) => {
  bot.action(`feat_${key}`, async (ctx) => {
    await ctx.answerCbQuery();
    const uid = ctx.from.id;
    if (!await ensureConnected(uid, ctx)) return;
...
bot.action("feat_ctcchecker", async (ctx) => {
  await ctx.answerCbQuery();
  if (!await ensureConnected(uid ?? ctx.from.id, ctx)) return;
...
bot.action(/^gs_similar_(.+)$/, async (ctx) => {
  ...
  try {
    const all = await getAllGroupsWithDetails(waIdx);
...
bot.action(/^gs_all_(.+)$/, async (ctx) => {
  ...
  const feature = ctx.match[1];
  try {
    const groups = await getAllGroupsWithDetails(waIdx);
...
bot.action("es_apply", async (ctx) => {
  ...
  for (let i = 0; i < total; i++) {
    ...
    await editProgress(ctx.chat.id, pm.message_id,
      `⚙️ Applying settings...\nDone: ${i}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
    try {
      const result = await withRetry(() => applyGroupSettings(waIdx, g.id, d), 2, 2000);
...
async function runChangeNameRandom(ctx, links, baseName, numbering) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      ...
      await withTimeout(withRetry(() => renameGroup(waIdx, info.id, newName), 2, 1500), 12000, "RenameGroup");
...
async function runChangeNameAsVcfAuto(ctx, vcfList) {
  ...
  let allGroups;
  try {
    allGroups = await withRetry(() => getAllGroupsWithDetails(waIdx));
  } catch (err) {
  ...
  for (const v of vcfList) {
    ...
    const resolved = phones.length ? await resolveVcfPhones(waIdx, phones) : [];
    ...
    try {
      ...
      try {
        const { jids: pendingJids, phones: pendingPhones } =
          await withTimeout(withRetry(() => getPendingRawJids(waIdx, g.id), 2, 1500), 10000, "PendingJids");
      ...
      if (bestVcf && bestCount > 0) {
        await withTimeout(withRetry(() => renameGroup(waIdx, g.id, bestVcf.name), 2, 1500), 12000, "RenameGroup");
        renamed++;
...
async function runFeature(ctx, feature, selectedIds, allGroups, extraNums) {
  const uid    = ctx.from.id;
  const waIdx  = await uidxAsync(uid);   // capture once — guaranteed correct index
  ...
  if (feature === "get_links") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `🔗 Getting links...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try { results.push({ name: g.name, link: await withRetry(() => getGroupInviteLink(waIdx, g.id)) }); done++; }
  ...
  if (feature === "leave") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `🚪 Leaving groups...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        await withRetry(() => leaveGroup(waIdx, g.id));
        done++;
      } catch (err) { failed++; }
  ...
  if (feature === "remove_members") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      try {
        ...
        const n = await withRetry(() => removeAllMembers(waIdx, g.id, 1, true));
  ...
  if (feature === "make_admin") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `👑 Making admin...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        const n = await makeAdminByNumbers(waIdx, g.id, extraNums);
        totalProm += n; done++;
  ...
  if (feature === "demote_admin") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `⬇️ Demoting admins...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        const n = await demoteAdminInGroup(waIdx, g.id, extraNums);
        totalDem += n; done++;
  ...
  if (feature === "reset_link") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `🔄 Resetting links...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try { results.push({ name: g.name, link: await withRetry(() => resetGroupInviteLink(waIdx, g.id)) }); done++; }
  ...
  if (feature === "approval") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `🔀 Toggling approval...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        const cur = await withRetry(() => getGroupApprovalStatus(waIdx, g.id)), next = !cur;
        await withRetry(() => setGroupApproval(waIdx, g.id, next));
        done++;
      } catch (err) { failed++; }
      if (await cancellableSleep(D.approvalToggle, uid)) { cancelled = true; break; }
    }
  ...
  if (feature === "approve_pending") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `✅ Approving pending...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        const r = await withRetry(() => approveAllPending(waIdx, g.id), 2, 5000);
  ...
  if (feature === "member_list") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `📋 Member list...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        const members = await withRetry(() => getGroupMembers(waIdx, g.id));
  ...
  if (feature === "pending_list") {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await editProgress(ctx.chat.id, pm.message_id,
        `⏳ Pending list...\nDone: ${done}/${total}  ❌ ${failed}\n→ ${g.name}\n${bar(i, total)}`);
      try {
        const { list: pending } = await withRetry(() => getGroupPendingRequests(waIdx, g.id));
...
bot.action("gf_create_now",async(ctx)=>{
  ...
  if (!await ensureConnected(uid, ctx)) return;
  const waIdx = await uidxAsync(uid);   // capture once — guaranteed correct
  ...
  for(let i=0;i<flow.count;i++){
    ...
    try{
      const r=await withRetry(()=>createGroup(waIdx,gname,jids));const gid=r.id;
      await sleep(3000); // give WA time to register the new group
      if(flow.description){await updateGroupDescription(waIdx,gid,flow.description).catch(()=>{});await sleep(600);}
      if(flow.photo){
        ...
        await withRetry(()=>updateGroupPhoto(waIdx,gid,photoBuf),3,3000).catch(()=>{});
        await sleep(800);
      }
      if(flow.disappearing){await setDisappearingMessages(waIdx,gid,flow.disappearing).catch(()=>{});await sleep(500);}
      if(flow.makeAdmin&&jids.length){await makeAdminByNumbers(waIdx,gid,flow.members).catch(()=>{});await sleep(1000);}
      await setGroupPermissions(waIdx,gid,flow.permissions).catch(()=>{});
      let link="";try{link=await getGroupInviteLink(waIdx,gid);}catch{link="(unavailable)";}
...
async function runAddMembersFromVcfs(ctx) {
  ...
  for (let i=0; i<total; i++) {
    ...
    try {
      ...
      const result = await addMembersToGroup(waIdx, info.id, contacts.map(c=>c.phone), flow.addMode==="onebyone");
...
bot.on("text", async (ctx) => {
  ...
  if (s.featureFlow?.step === "similar_query") {
    ...
    try {
      const allGroups = s.featureFlow.allGroups?.length ? s.featureFlow.allGroups : await getAllGroupsWithDetails(waIdx);
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      ...
      const { jids: pendingJids, phones: pendingPhones, count: pendingCount } =
        await withTimeout(withRetry(() => getPendingRawJids(waIdx, info.id), 2, 1500), 12000, "GetPendingJids");

      // Step 2: Resolve VCF phones via onWhatsApp to get real JIDs + LIDs
      const resolved = vcfPhones.length
        ? await withTimeout(resolveVcfPhones(waIdx, vcfPhones), 20000, "ResolveVcf")
        : [];
...
app.get("/health", (_, res) => res.json({ status:"ok", activeConnections:getConnectedCount(), ts:new Date().toISOString() }));
